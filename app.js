...
(function () {
  const ICONS = {
    ...
    "edit-2": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5z"/></svg>`,
    ...
    "more-horizontal": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>`,
    ...
    "save": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15.2 3a2 2 0 0 1 1.4.6l3.8 3.8a2 2 0 0 1 .6 1.4V19a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z"/><path d="M17 21v-7a1 1 0 0 0-1-1H8a1 1 0 0 0-1 1v7"/><path d="M7 3v4a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V3"/></svg>`,
  ...
  window.YTIcons = ICONS;

  function icon(name, className = "") {
    const svg = ICONS[name] || "";
    return svg.replace("<svg", `<svg class="${className}"`.trim());
  }
  ...
  function showToast(message, type = "success") {
    ...
    toast.innerHTML = `${icon(type === 'success' ? 'check' : 'x', 'nav-icon')}<span>${message}</span>`;
  ...
  function closeModal(selector) {
    ...
    if (!document.querySelector('.overlay.open, .drawer.open')) {
  ...
  window.YTModal = { open: openModal, close: closeModal };
  window.YTDrawer = { open: openDrawer, close: closeDrawer };

  // Close on overlay click or Escape
  document.addEventListener('click', (e) => {
    if (e.target.classList.contains('overlay')) {
      e.target.classList.remove('open');
      if (!document.querySelector('.overlay.open, .drawer.open')) document.body.style.overflow = '';
    }
  });

  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      document.querySelectorAll('.overlay.open, .drawer.open').forEach((el) => el.classList.remove('open'));
      document.body.style.overflow = '';
    }
  });
  ...
  window.YTCopy = copyText;

  // Export HTML blob helper
  ...
  window.YTDownload = downloadFile;
